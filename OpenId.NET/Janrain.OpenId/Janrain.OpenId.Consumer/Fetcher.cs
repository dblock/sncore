...
namespace Janrain.OpenId.Consumer
{
    ...
    {
	public static uint MAX_BYTES = 1024 * 1024; // 1MB

	protected static int ReadData ( HttpWebResponse resp, 
					uint max_bytes, 
					out byte[] buffer )
	{
	...
	protected static FetchResponse GetResponse ( HttpWebResponse resp,
						     uint maxRead )
	{
	    byte[] data;
	    int length = ReadData(resp, maxRead, out data);
	    return new FetchResponse(resp.StatusCode, resp.ResponseUri,
				     resp.CharacterSet, data, length);
	}

	public abstract FetchResponse Get ( Uri uri, 
					    uint maxRead );
	...
	public abstract FetchResponse Post ( Uri uri, 
					     byte[] body, 
					     uint maxRead );

	public virtual FetchResponse Post ( Uri uri, 
					    byte[] body )
	{
